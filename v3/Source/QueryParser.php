...
class QueryParser {
	...
	public function if( Models\Query $query ) {
		...
		$value = $query->getValue();
		$cacheData = array( $query->getCommandSignature(),  )
		...
		if ( !$query->isPostQuery() ) {

			$query->setCacheData( $query->getKey(), $query->getValue() );

			return new Models\Response( $area["full"], "", $query );
		}

		// return empty value when an else catched this if
		// or this if condition fails
		$elseValue = $query->getCacheData( "else-{$query->getKey("")}");

		if ( !is_null($elseValue) && !$elseValue || !$query->getValue() )
			return new Models\Response( $area["full"], "" );
		
		// else parse this if
		$content = \Templax\Templax::parse( $area["template"], (array) $query->getCommandValue(), array(), $query->getProcess() );
